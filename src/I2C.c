...
I2C_DeviceT I2C_DEV_1, I2C_DEV_2;
...
int i2c_read(int fd, unsigned char *read_data)
{
  ...
  if(ret == -1)
    max_error("I2C: Failed to read");
  if(ret == 0)
    max_error("I2C: End of FILE");
  return(ret);
}
...
int init_i2c_dev1(unsigned char slave_address)
{
  ...
  {
    max_error("I2C: Failed to open device");
    return -1;
  }
  if(Set_slave_addr(I2C_DEV_1.fd_i2c, I2C_DEV_1.i2c_slave_addr) == -1)
  {
    max_error("I2C: Failed to connect to slave device");
    return -1;
  }
